...
class Product:
    ...
    def from_dict(cls, data):
        return cls(data['product_id'], data['name'], float(data['price']), int(data['stock_quantity']))
...
class Sale:
    def __init__(self, items, total_amount, sale_datetime):
        self.items = items
    ...
    def to_dict(self):
        return {
            ...
            'items': ';'.join([f"{item.product.product_id}:{item.quantity}" for item in self.items])
        }
...
class InventoryManager:
    ...
    def add_product(self, product_id, name, price, stock_quantity):
        ...
        try:
            ...
            print("Price must be a number and stock quantity must be an integer!")
            return False
    ...
    def update_product(self, product_id, name=None, price=None, stock_quantity=None):
        ...
        if stock_quantity is not None:
            try:
                ...
                print("Stock quantity must be an integer!")
                return False
                        ...
                        try:
                            ...
                            if items_data:
                                for item_str in items_data.split(';'):
                                    if ':' in item_str:
                                        product_id, quantity = item_str.split(':')
                                        product = self.inventory_manager.get_product(product_id)
                                        if product:
                                            items.append(OrderItem(product, int(quantity)))
                            
                            sale_datetime = datetime.fromisoformat(row['datetime'])
                            total_amount = float(row['total_amount'])
                            sale = Sale(items, total_amount, sale_datetime)
                            sales.append(sale)
                        except (ValueError, KeyError) as e:
    ...
    def save_sales(self):
        try:
            with open(self.sales_file, 'w', newline='', encoding='utf-8') as file:
                fieldnames = ['datetime', 'total_amount', 'items']
                writer = csv.DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
    ...
    def checkout(self):
        ...
        for item in self.cart:
            ...
            if product.stock_quantity < item.quantity:
                print(f"Error: {product.name} has only {product.stock_quantity} in stock, but {item.quantity} requested!")
                return None
        ...
        success = self.save_sales() and self.inventory_manager.save_data()
        ...
        if success:
            ...
            print("Error during checkout! Please try again.")
            return None
...
def product_management_menu(inventory_manager):
    while True:
        ...
        if choice == '1':
            ...
            print(f"Current: Name='{current.name}', Price=${current.price:.2f}, Stock={current.stock_quantity}")
            ...
            stock_quantity = stock_str if stock_str else None
            ...
            if sale:
                ...
                if save_bill == 'y':
                    ...
                    if format_choice in ['txt', 'csv']:
                        ...
                        print("Invalid format! Using default (txt).")
                        billing_system.generate_bill(sale)
